...
import { BankingWelcomeStep } from './BankingWelcomeStep';
import { BankingGeneralStep } from './BankingGeneralStep';
import { BankingDetailsStep } from './BankingDetailsStep';
import { BankingReviewStep } from './BankingReviewStep';
import { BankingSuccessStep } from './BankingSuccessStep';

import { BANKING_ENABLED } from './constants';

export default function BankingApp() {
  // ── Session ID resolution — same pattern as every other product flow ──────
  const STORAGE_KEY = 'albie_banking_session_id';
  const slugMatch = window.location.pathname.match(/^\/banking\/o\/(.+)$/);
  ...
  const [sessionId, setSessionId] = useState<string | null>(() => {
    ...
    const newId = `bank_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
  ...
  useEffect(() => {
    ...
    document.title = 'Banking Information | ALBIE by TAG';
  ...
  const firstModule = 1;
  const reviewStep = firstModule + BANKING_ENABLED.length;
  ...
  const currentModuleId = isModuleStep ? BANKING_ENABLED[currentStep - firstModule] : null;

  const progressCurrent = isModuleStep ? currentStep - firstModule + 1 : BANKING_ENABLED.length + 1;
  const progressTotal = BANKING_ENABLED.length + 2;
  ...
  const buildPayload = (forms = savedForms) => ({
    sessionId,
    product: 'banking' as const,
    general: forms.general ?? {},
    banking: forms.banking ?? {},
  });
  ...
  const handleEditModule = (moduleId: string) => {
    const index = BANKING_ENABLED.indexOf(moduleId);
    if (index < 0) return;
    window.scrollTo(0, 0);
    ...
    if (currentStep === reviewStep) {
      ...
      try {
        ...
        fetch('/api/send-onboarding', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ ...finalPayload, product: 'banking' }),
  ...
  const moduleComponents: Record<string, ReactNode> = {
    general: <BankingGeneralStep prefill={savedForms.general ?? {}} />,
    banking: <BankingDetailsStep prefill={savedForms.banking ?? {}} />,
  };
        ...
        <AnimatePresence mode="wait">
          ...
          {currentStep === 0 && (
            <motion.div key="welcome" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 1.05 }} className="h-full">
              <BankingWelcomeStep onNext={goNext} propertyName={onboardingName} />
            </motion.div>
          )}
          ...
          {currentStep === successStep && (
            ...
            >
              <BankingSuccessStep engagementSlug={engagementSlug} />
            </motion.div>
          )}
